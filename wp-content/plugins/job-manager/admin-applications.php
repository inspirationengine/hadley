...
function jobman_application_details_layout( $appid ) {
	...
	if( array_key_exists( 'jobman-email', $_REQUEST ) ) {
        ...
        if(is_email($_REQUEST['jobman-email']))
...
?>
            ...
            <?php
                if (count($arrInvited)>0){ ?>
                    ...
                    <?php
                        $strInvited = '';
                        foreach ($arrInvited as $row){
                            $strInvited.= '<tr><td>' .$row->email. "</td><td style='color: green;'>Sent</td>";
                        }
                        if (array_key_exists( 'jobman-email', $_REQUEST) && !is_email($_REQUEST['jobman-email'])){
                            $strInvited.= '<tr><td>' .$_REQUEST['jobman-email']. "</td><td style='color: red;'>Incorrect email</td>";
                        }
                        echo $strInvited;
...
function insert_invited_colleague($applicant_id, $email){
    GLOBAL $wpdb;
